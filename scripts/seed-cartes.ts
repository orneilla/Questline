 ...
 *
 *   node --env-file-if-exists=.env --import tsx scripts/seed-cartes.ts
 *
 * Deux cents cartes réparties sur plusieurs espaces et paquets, à des stades
 * de maturité différents, avec du LaTeX, de l'arabe et une image. De quoi
 * peupler le jardin et éprouver les performances.
 *
 * Refuse de tourner si des cartes existent déjà ; FORCE=1 pour remplacer.
 */
import { db } from "../src/db";
import { cartes, espaces, paquets, revisions } from "../src/db/schema";
import { engendrer, type NoteSource } from "../src/lib/cartes/generation";
import { aujourdhui, decalerJours } from "../src/lib/dates";
...
async function semer() {
  const existantes = await db.select({ id: cartes.id }).from(cartes).limit(1);
  if (existantes.length > 0 && process.env.FORCE !== "1") {
  ...
  console.log(
    `${CATALOGUE.length} espaces, ${CATALOGUE.reduce((t, e) => t + e.paquets.length, 0)} paquets, ` +
      `${totalCartes} cartes, ${totalRevisions} révisions.`,
  );
}
